...
class GoogLeNet(nn.Module):#总框架
    def __init__(self, num_classes=1000, aux_logits=True, init_weights=False):#分类个数，是否使用辅助分类器，是否对权重初始化
...
class Inception(nn.Module):
    ...
    def forward(self, x):#前向传播，x为输入特征矩阵
        ...
        return torch.cat(outputs, 1)#四个分支的输出合并,1对应tensor的channel
...
class InceptionAux(nn.Module):#辅助分类器
    ...
    def forward(self, x):#正向传播
        ...
        x = F.dropout(x, 0.5, training=self.training)#0.5 50%比例(原文70%，效果没有50%好) training= 在训练模式下为ture 测试模式下为f
        # N x 2048
